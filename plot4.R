...
#
# specify multiple base plots 2 x 2 (filled column-wise)
#
par(mfcol=c(2,2))
#
# plot nº 1
# construct the time (first column) plot with data in Global_active_power column (now is 4th column)
#
plot(table[,1],table[,4],type="l",xlab="Date_Time",ylab="Global Active Power (kilowatts)")
#
# plot nº 2
# construct 3 plots: Date_Time (first column) vs. Sub_metering_(1,2 and 3)
#
 with(table, plot(Date_Time, Sub_metering_1, col="black",type="l", ylab="Energy sub metering (watt-hour)"))
 with(table, points(Date_Time, Sub_metering_2, col="red",type="l"))
 with(table, points(Date_Time, Sub_metering_3, col="blue",type="l"))
#
# add legend
#
legend(table[800,1], 38.5, c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c("black","red","blue"), text.col = "black", lty = c(1, 1, 1), pch = c(NA, NA, NA), merge = TRUE, box.col="white")
#
# plot nº 3
# construct plot: Date_Time (first column) vs. Voltage
#
 with(table, plot(Date_Time, Voltage, col="black",type="l", ylab="Voltage (volts)"))
#
# plot nº 4
# construct plot: Date_Time (first column) vs. Global_reactive_power
#
 with(table, plot(Date_Time, Global_reactive_power, col="black",type="l", ylab="Global Reactive Power (kilowatts)"))
